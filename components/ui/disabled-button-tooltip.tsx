...
export function DisabledButtonTooltip({
  ...
  const flexClass = flexMatch ? flexMatch[1] : ''
  // Remove flex-1/flex-[2] from button className to avoid conflicts, button will fill wrapper
  const cleanedButtonClassName = buttonClassName.replace(/\b(flex-\d+|flex-\[.*?\])\b/g, '').trim()
  
  // Clone children with cleaned className and make it fill the wrapper
  const buttonWithCleanedClassName = React.cloneElement(children, {
    className: `${cleanedButtonClassName} flex-1`,
  })
      ...
      <Tooltip delayDuration={300}>
        <TooltipTrigger asChild>
          <div className={`flex items-center gap-1.5 ${flexClass}`}>
            {buttonWithCleanedClassName}
            <Info className="h-3.5 w-3.5 text-muted-foreground flex-shrink-0" aria-hidden="true" />
          </div>
        </TooltipTrigger>
