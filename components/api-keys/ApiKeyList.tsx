    ...
    <AutoSkeleton isLoading={isLoading}>
      ...
      {keys.length === 0 ? (
        <EmptyState
          ...
          description="Create an API key to access the Bulk GPT API programmatically"
          action={{
          ...
          {keys.map((key) => (
            <div key={key.id} className="p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 hover:bg-secondary/30 transition-colors">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-red-400 hover:text-red-300 hover:bg-red-500/10 rounded transition-colors disabled:opacity-50 self-start sm:self-auto"
